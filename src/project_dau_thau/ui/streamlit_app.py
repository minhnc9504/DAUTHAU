...
from project_dau_thau.data.store import ContractorStore, TenderStore
...
from project_dau_thau.services.competitor import analyze_competitors
from project_dau_thau.services.profile import get_profile, load_profiles
from project_dau_thau.settings import SETTINGS
import joblib
...
APP_TITLE = "🚀 Hệ thống Gợi ý gói thầu phù hợp cho Doanh nghiệp v12.0"


def render():
    st.set_page_config(
        page_title="Gợi ý gói thầu phù hợp cho DN v12.0",
        layout="wide",
    )
...
def _render_profile_tab(contractor_store: ContractorStore, profiles_df):
    ...
    # Pre-select company if coming from session (jump from "Gợi ý gói thầu")
    default_idx = 0
    if (
    ...
    # Lý do chỉ hiển thị cho các gói KHÔNG trúng
    def _get_reason(row):
    ...
    # st.markdown("---")
    # if st.button("🎯 Gợi ý gói thầu cho doanh nghiệp này", use_container_width=True, key="btn_goi_y_profile"):
    #     st.session_state.pending_company = selected
    #     st.session_state.jump_to_tab = "recommend"
    #     st.rerun()


# ────────────────────────────────────────────────────────────────
# Tab 2: Gợi ý gói thầu (tích hợp phân tích đối thủ)
# ────────────────────────────────────────────────────────────────
...
def _render_recommendation_tab(tender_store, ranker, contractor_store, profiles_df, sem_ready):
    ...
    # Hiển thị kết quả + phân tích đối thủ bên dưới
...
def _do_recommend(tender_store, ranker, contractor_store, profiles_df,
    ...
    # Apply date filter: loại bỏ gói đã đóng (decision/open/public date đã qua)
    ...
    if orgfullname and profiles_df is not None:
        ...
        if profile:
            for r in filtered:
                ...
                if r.lexical_score > 40:
                    parts.append("Đúng ngành nghề")
                if r.historical_fit_score > 60:
                    parts.append("Phù hợp lịch sử")
                if r.price_fit_score > 80:
                    parts.append("Khung giá phù hợp")
                if r.recency_score > 80:
...
def _display_recommendation_with_competitors(results, ranker, contractor_store):
    """Hiển thị kết quả gợi ý + phân tích đối thủ bên dưới."""
    if not results:
    ...
    if selected_tender and contractor_store is not None:
        ...
        # Tìm ranked tender info
        sel_ranked = next((r for r in results if r.notifyno == notifyno), None)

        # Loại trừ công ty hiện tại (nếu đã chọn ở tab gợi ý)
        col1, col2 = st.columns(2)
        with col1:
