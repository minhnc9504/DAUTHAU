...
COMPETITOR_SCORE_THRESHOLD = 1.0
...
def analyze_competitors(
    ...
    def safe_rate_col(series_win, series_join) -> pd.Series:
        ...
        multiplier = SETTINGS.competitor_rate_multiplier
        return vals.clip(upper=1.0) * multiplier
    ...
    # Lọc: tổng tham gia >= min_join
    res["total_join"] = (
        res["join_inv"] + res["join_prov"] + res["join_f"] + res["join_seg"]
    )
    from settings import SETTINGS
    min_join = SETTINGS.competitor_min_join
    res = res[res["total_join"] >= min_join]
